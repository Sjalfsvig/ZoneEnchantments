package me.sjalfsvig.zoneenchantments.util.api;
...
import me.sjalfsvig.zoneenchantments.util.api.ArmorEquipEvent.EquipMethod;
...
public class ArmorEquipListener implements Listener {
	...
	public final void onInventoryClick(final InventoryClickEvent e){
		...
		if(shift){
			...
			if(newArmorType != null){
				boolean equipping = true; // equipping if its a piece of armor
	...
	// This shit didn't work idk, making my own.
	...
	public void onPlayerInteract(PlayerInteractEvent event) {
		if (event.getAction() == Action.RIGHT_CLICK_AIR || event.getAction() == Action.RIGHT_CLICK_BLOCK) {
			...
			if (event.getClickedBlock() != null) {
				...
				if (newArmorType != null) {
					if(newArmorType.equals(ArmorType.HELMET) && event.getPlayer().getInventory().getHelmet() == null || newArmorType.equals(ArmorType.CHESTPLATE) && event.getPlayer().getInventory().getChestplate() == null || newArmorType.equals(ArmorType.LEGGINGS) && event.getPlayer().getInventory().getLeggings() == null || newArmorType.equals(ArmorType.BOOTS) && event.getPlayer().getInventory().getBoots() == null){
						ArmorEquipEvent armorEquipEvent = new ArmorEquipEvent(event.getPlayer(), EquipMethod.HOTBAR, ArmorType.matchType(event.getItem()), null, event.getItem());
						Bukkit.getServer().getPluginManager().callEvent(armorEquipEvent);
						if(armorEquipEvent.isCancelled()){
